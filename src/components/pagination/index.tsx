...
const Step = (props: StepProps) => {
  switch (props.variant) {
    case 'link':
      return (
        <Link href={props.href(props.page)} className={styles.step} aria-label={props.label} rel={props.rel} tone="muted" underline={false}>
          {props.glyph}
        </Link>
      );
    case 'disabled':
...
const PageLink = ({ page, current, href }: PageLinkProps) => (
  <Link href={href(page)} className={styles.link} aria-current={current ? 'page' : undefined} data-current={current ? 'true' : undefined} tone="muted" underline={false}>
    {page}
  </Link>
);
