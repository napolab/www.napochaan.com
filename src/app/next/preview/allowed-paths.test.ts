...
describe('isAllowedPreviewPath', () => {
  ...
  it('open-redirect(protocol-relative URL)を拒否する', () => {
    ...
    expect(isAllowedPreviewPath('https://evil.com/works/preview/1')).toBe(false);
  });

  it('preview 接頭辞に合致しないパスを拒否する', () => {
    expect(isAllowedPreviewPath('/works')).toBe(false);
    expect(isAllowedPreviewPath('/works/foo')).toBe(false);
