...
| `TITLE_JP_VAR` (src/themes/font-vars, imported directly by `titleJP` token) ↔ `variable` literal in `Zen_Kaku_Gothic_New(...)` (src/themes/fonts-title.ts — next/font requires a literal, can't import the constant) | `src/themes/font-vars.test.ts` (statically reads fonts-title.ts source; the module can't be imported under vitest — next/font/google is a build-time-only stub outside Next's compiler) |
