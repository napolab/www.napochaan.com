...
| `create_post`  | `title`, `slug`, `excerpt`, `thumbnailMediaID`, `bodyMarkdown`, `publishedAt?` | **常に draft** で作成。`slug` は必須(auto 生成なし・小文字英数字とハイフン)。`publishedAt` 省略時は今日(Asia/Tokyo、`@utils/dayjs`)。admin URL を返す |
