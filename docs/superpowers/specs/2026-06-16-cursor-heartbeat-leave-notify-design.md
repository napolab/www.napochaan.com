...
**durabcast の Alarm を壊さないための制約**:

- override 側で `setAlarm` / `deleteAlarm` を**一切呼ばない**。close も alarm 再武装（`setAlarm(+INTERVAL)`）も `super.alarm()` が単独所有する。
- override の冒頭に `if (!this.AUTO_CLOSE) { await super.alarm(); return; }` のガードを置き、基底の reaping 条件に厳密追従する。auto-close が無効なら基底は何も閉じないので、こちらも何も通知せず委譲だけする（閉じられないソケットに leave を撒く不整合を防ぐ）。`CursorRoom` は既定 options（AUTO_CLOSE = true）のままなので通常は通過する。
- `super.alarm()` を**必ず最後に await する**（省略すると刈り取りも再武装も止まる）。テストで「刈り取り後に alarm が再武装されている」ことを直接検証し、heartbeat を壊していないことを担保する。
...
4. **durabcast の Alarm を壊していない**: 刈り取り後にピアが残っていれば alarm が再武装されている（`runInDurableObject` で `state.storage.getAlarm()` が非 null）。`super.alarm()` の委譲が効いていることの直接検証。
