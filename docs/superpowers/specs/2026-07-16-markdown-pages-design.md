...
frontmatter（`title` / `url`）+ プロフィール（name / aka / now / team / tagline）+ bio / philosophy を `lexicalToMarkdown` で本文化し、続けて love / skill / contact の各セクション（`## love` / `## skill` / `## contact`、空配列なら該当セクションごと省略）を追加。
