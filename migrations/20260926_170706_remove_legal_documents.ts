...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  // Drop admin edit-locks held on legal documents first (rels rows cascade), so the
  // rebuilt rels table carries no orphaned lock rows.
  await db.run(sql`DELETE FROM \`payload_locked_documents\` WHERE \`id\` IN (SELECT \`parent_id\` FROM \`payload_locked_documents_rels\` WHERE \`legal_documents_id\` IS NOT NULL);`)
  ...
  // Drop the child (versions, FK → legal_documents) before its parent, and only after
  // payload_locked_documents_rels no longer references legal_documents.
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  // Restores 20260720_155000_legal_documents_autosave (hand-written, so absent from the snapshot).
